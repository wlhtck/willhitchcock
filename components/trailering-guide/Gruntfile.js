...
module.exports = function(grunt) {
    ...
    grunt.initConfig({
        ...
        uglify: {
            dist: {
                src: ['<%= pkg.directory_source %>/js/trailering-guide.js'],
                dest: '<%= pkg.directory_source %>/js/trailering-guide.min.js'
            }
        },
        concat: {
            dist: {
                src:'<%= pkg.directory_source %>/js/trailering-guide.min.js',
                dest: '<%= pkg.directory_build %>/js/trailering-guide.min.js'
            }
        },
        ...
        sass: {
          dist: {
            ...
            files: {
              '<%= pkg.directory_source %>/css/trailering-guide.css': '<%= pkg.directory_source %>/sass/main.scss'
            }
          }
        },
        cssmin: {
    ...
    process.chdir(cwd);
    // Default task.
    grunt.registerTask('default', ['clean', 'uglify', 'concat', 'sass', 'cssmin', 'copy', 'processhtml']);
};